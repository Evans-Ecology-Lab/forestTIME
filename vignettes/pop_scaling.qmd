...
E.g. the variance *estimator* for per-acre ratios (without post-stratification weights) is equation 4.17 in @bechtold2005 which I'm fairly certain is *not* the same as the formula the `var()` function uses to calculate [variance of a random variable](https://en.wikipedia.org/wiki/Variance):
